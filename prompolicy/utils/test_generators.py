...
def test_metricsfactory():
    ...
    assert len(m) == 6
    for mm in m:
        assert isinstance(mm, Metric2Cerbos)
    for mm in m.to_grpc:
        assert isinstance(mm, Metric2Cerbos)
...
def test_metricsfactory_from_response():
    data = json.loads(
        '{"status":"success","data":{"resultType":"matrix","result":[{"metric":{"__name__":"node_cpu_seconds_total","cluster":"us-east-1","cpu":"0","instance":"retropie.chester.at:9100","job":"node","mode":"idle"},"values":[1,2,3,4,5]}]}}'
    )
    mdata = data.get("data")
